...
export default function EditAvailability() {
  ...
  const {
    ...
    hasDraft,
    draftSource
  } = useAvailabilityDraft(initialFormData, `availabilityEditDraft_${params.id}`);
  ...
  useEffect(() => {
    ...
    if (params.id) {
      ...
      if (draft) {
        ...
        console.log('📂 Restoring availability edit draft from sessionStorage');
  ...
  }, [user, authLoading, router, params.id, loadDraft]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Clear the draft since availability is now saved
      clearDraft();
